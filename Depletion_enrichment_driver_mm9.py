...
def get_uces(args):
    ...
    aUCEFiles = []
    if args.all:
        if os.path.isfile('mm9.genomic.coordinates.nonN'):
            allLen = len(list(args.all))
            aUCEFiles.append(('all', args.all.name, 'mm9.genomic.coordinates.nonN', allLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.all.name)
            sys.exit(1)
    if args.intergenic:
        if os.path.isfile('mm9_intergenic_1based.bed'):
            interLen = len(list(args.intergenic))
            aUCEFiles.append(('intergenic', args.intergenic.name, 'Intergenic_1based.bed', interLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.intergenic.name)
            sys.exit(1)
    if args.intronic:
        if os.path.isfile('mm9_introns_1based.bed'):
            inLen = len(list(args.intronic))
            aUCEFiles.append(('intronic', args.intronic.name, 'Introns_1based.bed', inLen))
        else:
            print "Cannot find appropriate spacing file for {0}, exiting...".format(args.intronic.name)
            sys.exit(1)
    if args.exonic:
        if os.path.isfile('mm9_exons_1based.bed'):
            exLen = len(list(args.exonic))
            aUCEFiles.append(('exonic', args.exonic.name, 'Exons_1based.bed', exLen))
        else:
